...
const questions: Question[] = [
  ...
  { id: 'challenge_27', original_word: 'public static int contarVogais(String str) { int count = 0; String vogais = "aeiouAEIOU"; for (int i = 0; i < str.length(); i++) { if (vogais.indexOf(str.charAt(i)) != -1) count++; } return count; }', question: 'Monte a sequência adequada para uma função estática que conta o número de vogais em uma string.', difficulty: 2, points: 200 },
  { id: 'challenge_28', original_word: 'private static void imprimirPadrao(int n) { for (int i = 1; i <= n; i++) { for (int j = 1; j <= i; j++) { System.out.print(j + " "); } System.out.println(); } }', question: 'Ordene as palavras para formar uma função estática que imprime um padrão numérico.', difficulty: 3, points: 300 },
  { id: 'challenge_29', original_word: 'public static String converterParaMaiusculo(String str) { return str.toUpperCase(); }', question: 'Disponha as palavras na ordem correta para uma função estática que converte uma string para maiúsculas.', difficulty: 1, points: 100 },
  { id: 'challenge_30', original_word: 'private static int somarArray(int[] arr) { int sum = 0; for (int num : arr) { sum += num; } return sum; }', question: 'Coloque as palavras na ordem correta para uma função estática que calcula a soma dos elementos de um array de inteiros.', difficulty: 2, points: 200 }
];
